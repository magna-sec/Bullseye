...
resource "proxmox_vm_qemu" "blog_demo_test" {
  #name = "test_vm${count.index}" # count.index starts at 0
  name = "test-vm-01"
  ...
  full_clone  = "true"
  #iso = "/var/lib/vz/template/iso/ubuntu-20.04.6-live-server-amd64.iso"
  ...
  disk {
    ...
    size = "50G"
    type = "scsi"
    storage = "nas-backups" # Name of storage local to the host you are spinning the VM up on
